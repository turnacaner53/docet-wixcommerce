...
export async function generateMetadata({
  params: { slug },
  searchParams: { page },
}: CollectionsPageRouteProps): Promise<Metadata> {
  ...
  return (
    <div className='space-y-5'>
      ...
      <Suspense fallback={<LoadingSkeleton />} key={page}>
        <Products page={parseInt(page)} collectionId={collection._id} />
      </Suspense>
    </div>
  );
}
...
async function Products({ collectionId, page }: ProductsProps) {
  ...
  if (!collectionProducts.length) return notFound();
