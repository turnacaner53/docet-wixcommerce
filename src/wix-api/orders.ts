...
export async function getOrder(wixClient: WixClient, orderId: string) {
  try {
    ...
    if ((error as any).details.applicationError.code === 'NOT_FOUND') return null;
    else throw error;
  }
}
...
export async function getUserOrders(wixClient: WixClient, { limit, cursor }: GetUserOrderFilters) {
